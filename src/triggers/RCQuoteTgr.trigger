/************************************************************************************************************
* UPDATE:                                                                                                   *
* Case #: Case: 03987840 - Changes on the Quote Tool UI                                                     *
* Description:                                                                                              *
*       1. Add Payment Period in RC Quote, Quote (Sales Agreement) field. Display in UI Quote and           *
*       Agreement tab. Make the field Required.                                                             *
*       2. Remove the Auto Renewal field.                                                                   *
*       3. Make the Renewal Terms field required.                                                           *
*       4. Pass the value of Payment Period from RC QUote to Quote                                          *
*                                                                                                           *
...
trigger RCQuoteTgr on RC_Quote__c (before insert, before update, after update, before delete) {
    ...
    if(trigger.isAfter &&  trigger.isUpdate){
        ...
        /****************  Insert & Update Sales Agreement Record on Status Changes of Quote Start**************************/
            ...
            // Case: 03987840 - Changes on the Quote Tool UI - START: Add the Payment_Period__c in the query
            List<RC_Quote__c> quoteObjList = [select Opportunity__c,End_Date__c, Opportunity__r.AccountId,Quote_Type__c, Agreement_Status__c,Total_MRR__c, 
                                                    Start_Date__c, Total_Initial_Amount__c,Total_12M_Amount__c,Name, Renewal_Term__c, Initial_Term__c, Payment_Period__c, 
                                                    (Select Id, Status From Quotes__r where status = 'Active' limit 1) 
                                                    from RC_Quote__c
                                                    where Id IN : Trigger.newmap.keyset()];
            // Case: 03987840 - Changes on the Quote Tool UI - END
            ...
            for(RC_Quote__c quoteObj : trigger.new) {
                ...
                if(quotesMap != null && quotesMap.get(quoteObj.id).Quote_Type__c == 'Agreement' && quotesMap.get(quoteObj.id).Agreement_Status__c == 'Active' && Trigger.oldMap.get(quoteObj.id).Agreement_Status__c != 'Active') {
                    
                    if(quoteAccountMap != null && quotesMap.get(quoteObj.id).Opportunity__r.AccountId != null && quoteAccountMap.get(quotesMap.get(quoteObj.id).Opportunity__r.AccountId).size()== 0) {
                        ...
                        // Case: 03987840 - Changes on the Quote Tool UI - START: Pass the Payment_Period__c from RC Quote to Quote
                        quoteObjTOIns.Payment_Period__c = quoteObj.Payment_Period__c;
                        // Case: 03987840 - Changes on the Quote Tool UI - END
                ...
                } else if(quotesMap != null && quoteObj.Quote_Type__c == 'Agreement' && Trigger.oldMap.get(quoteObj.id).Agreement_Status__c == 'Active' && (quoteObj.Agreement_Status__c != 'Active' )) {
                    if(quotesMap.get(quoteObj.id).Quotes__r != null && quotesMap.get(quoteObj.id).Quotes__r.size() == 1) {
                        ...
                        // Case: 03987840 - Changes on the Quote Tool UI - START: Pass the Payment_Period__c from RC Quote to Quote
                        quoteObjTOUpd.Payment_Period__c = quoteObj.Payment_Period__c;
                        // Case: 03987840 - Changes on the Quote Tool UI - END
