...
trigger Account_Before on Account (before insert, before update,before delete) {
  ...
  if(trigger.isInsert || trigger.isUpdate){
    ...
    If(trigger.isUpdate){
      AccountTriggerHelperExt.Update_Account_CountOfOrder (trigger.newMap);
    }    
