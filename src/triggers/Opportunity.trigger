...
trigger Opportunity on Opportunity (before insert, before update) {
	/*********************** DFR CODE *******************************/
	Profile objpro = [SELECT Name ,Id FROM Profile WHERE Id =:UserInfo.getProfileId()]; // New Line Case #03793086
	if(Trigger.isUpdate  && trigger.isBefore){	          
	    for(Opportunity l : Trigger.new) {
           ...
           if(l.OwnerId != oldL.OwnerId && oldL.OwnerId != NULL 
           			&& TriggerHandler.IsByPassedValidaionOnOppty == false 
           			&& objpro.Name.equalsIgnoreCase('Channel Sales')){
           	  l.addError('You are not authorized to transfer this Opportunity.');
           }
    ...
    if(Trigger.isInsert) {
        //Code for Calculating Amount Based on RC Tier and Brand OR LEAD SOURCE. Added on 25th April,14
        OpportunityMethods.calculateAmountBasedOnCSV(Trigger.new);
    }
    if(Trigger.isUpdate) {
        //Code for Calculating Amount Based on RC Tier and Brand WHEN NO LINE ITEM IS THERE. Added on 9th November,14
        OpportunityMethods.calculateAmountForZeroLineItems(Trigger.new);
    }
    /***********************************************************************************************************/
    ...
    */
    ...
    if(trigger.isInsert && trigger.isBefore){
    	...
    	for(Opportunity NewOppObj :trigger.new ){
    		...
    		//;........This change added by simplion on 10 feb, 2015 ..If OppClose date is blank the it set closedate to today................ 
    		
    		if(NewOppObj.CloseDate!=null  && prof.Name!= 'System Administrator' ){
           	    NewOppObj.CloseDate = system.today();
            	system.debug('?*****************'+NewOppObj.CloseDate);
             }
             if(NewOppObj.CloseDate == null  ){
            	 NewOppObj.CloseDate = system.today();   
            	 system.debug('?***&&&&&&&&&**************'+NewOppObj.CloseDate);           	        
             }		
    		/*
	...
	if(!byPass){		
		...
		for(Opportunity NewOppObj :trigger.new ){
			...
			*/
			...
			//Update by simplin on 1/28/2015 to set closedate == today and allow to update by admin profile  
				if(prof.Name == 'System Administrator'  && OldOppObj.CloseDate != null){
	 				if(NewOppObj.closeDate != oldOppObj.closeDate){	 						
	 				}else {
	 					NewOppObj.CloseDate = system.today(); 
	 				 } 	 				
				}else{
					...
					*/	
					 ...
					 if(NewOppObj.RecordTypeId  == OpportunityHelper.getOppRecordTypeMap('Saves Opportunity') 
					 	&& NewOppObj.Brand_Name__c.contains('RingCentral')) {
			 				...
			 				// Commented for Case#03472417. We are no more creating SalesSurvey.
			...
			*/
			if((OpportunityHelper.isClosedOpp(NewOppObj.StageName) && OpportunityHelper.isClosedOpp(OldOppObj.StageName)) && (NewOppObj.CloseDate != OldOppObj.CloseDate)){
				...
				if(!'System Administrator'.equalsIgnoreCase(prof.Name)){
                 	NewOppObj.CloseDate = OldOppObj.CloseDate;
                 	NewOppObj.addError('You cannot modify the Close Date of a Closed Opportunity.');
            ...
            */
            if(OpportunityHelper.isActiveOpp(NewOppObj.StageName)){
            	Date todayDate = system.today();
            	Date oppDate = NewOppObj.closeDate;
            	Integer dateValues =  todayDate.daysBetween(oppDate);
            	//Integer dateValues =  todayDate.daysBetween(oppDate);
            	//datetime differenceDate = Date.today() - date.valueof(oppDate);
            	if(!'System Administrator'.equalsIgnoreCase(prof.Name)){           		
                 	if(oppDate < todayDate && NewOppObj.closeDate!= trigger.oldMap.get(NewOppObj.Id).closeDate ){
                 		NewOppObj.addError('You cannot set the Close Date to the past');
                 	}
                 }
            }
			/*
			...
			*/
			...
			// As on 17th April,2015, Deprecating Zuora code
