...
trigger Attachment on Attachment (before insert,before update,after insert, after update) {
	if(Trigger.isBefore){
	    ...
	    profList = [select Name from Profile where Id = :UserInfo.getProfileId()];
	    if (profList.size() >0){
	    	prof = profList.get(0);
	    }
	    ...
	    for(Attachment attVal : Trigger.New) {
	    	if(attVal != null && attVal.ParentId != null && string.valueOf(attVal.ParentId).startsWith('500')) {        
				if(caseMapPorting != null && caseMapPorting.get(attVal.ParentId) != null && prof.Name != 'System Administrator'  
					...
					&& !localToPartnerCaseSet.contains(attVal.ParentId)) {
